...
class MetricsCalculator:
    ...
    def _calculate_frame_based_availability(self, valid_frames: List[FrameMetric]) -> float:
        ...
        frames_by_second = self._group_frames_by_second(valid_frames, window_duration)
    ...
    def _group_frames_by_second(self, frames: List[FrameMetric], window_duration: int) -> Dict[int, List[FrameMetric]]:
        ...
        window_start = latest_ts - (window_duration * 1000)  # Convert to ms
        ...
        for frame in frames:
            if frame.received_ms >= window_start:
                ...
                seconds_from_start = int((frame.received_ms - window_start) // 1000)
